package com.closeplanet2.pandacommandcore.OBJECTS;

import com.closeplanet2.pandacommandcore.ENUMS.PlayerCommandError;
import com.closeplanet2.pandacommandcore.ENUMS.TabType;
import com.closeplanet2.pandacommandcore.INTERFACE.*;
import com.closeplanet2.pandacommandcore.PlayerCommand;
import com.closeplanet2.pandacommandcore.SignatureBuilder;
import com.closeplanet2.pandaspigotcore.PLAYER.PlayerAPI;
import com.closeplanet2.pandaspigotcore.PLAYER.TOGGLE_ACTIONS;
import com.closeplanet2.pandaspigotcore.STORED_DATA.PlayerDataAPI;
import com.closeplanet2.pandaspigotcore.STORED_DATA.ServerDataAPI;
import com.closeplanet2.pandavariablecore.API.VariableCoreAPI;
import com.closeplanet2.pandavariablecore.PandaVariableCore;
import com.closeplanet2.pandavariablecore.VAR_TESTS.STATIC_TESTS.StaticList;
import com.closeplanet2.pandavariablecore.VAR_TESTS.STATIC_TESTS.StaticString;
...
public class CustomPlayerMethod {
    ...
    public PlayerCommandError TryAndInvokeMethod(Player player, String[] player_args){
        ...
        for(var i = 0; i < expected_data_types.size(); i++){
            ...
            var all_types = VariableCoreAPI.RETURN_AS_ALL_TYPES(player_arg, false, toLower, type.isArray());
            if(!all_types.contains(name)) return PlayerCommandError.COMMAND_CANNOT_BE_INVOKED_WITH_PROVIDED_ARGUMENTS;
            var param_test = PandaVariableCore.VAR_TESTS.get(type);
    ...
    private List<String> ConvertTabToList(PCTab tab, String[] args, Player player, int pos, int numberOfArguments){
        ...
        if(tab.type() == TabType.Stored_Tab_Data_From_Type){
            var test = VariableCoreAPI.RETURN_TEST_FROM_TYPE(tab.data(), false);
